...
def _row_annotation(adata, cats, groupby, palette, annotate, annotate_palettes, annot_kws):
    ...
    annos = {
        "Group": pch.anno_simple(
            ...
            colors=primary_colors, add_text=False, legend=False,
        )
    }
    if annotate:
        ...
        per_group = sub.groupby(groupby, observed=True).agg(
            lambda v: v.mode().iloc[0] if not v.mode().empty else v.iloc[0]
        ).reindex(cats)
        for col in annotate:
            ...
            cpal = resolve_palette(
                annotate_palettes.get(col), uniq,
                sheet_name=col, adata=adata, groupby=col,
            )
            annos[col] = pch.anno_simple(
                pd.Series(ann_cats, index=cats, name=col),
                colors=cpal, add_text=False, legend=True,
            )
...
def complex_dotplot(
    ...
    **kwargs,
    ...
    cats = list(mean_df.index)
    long = (
        mean_df.stack().rename("value").reset_index()
        .rename(columns={mean_df.index.name or "level_0": groupby, "level_1": "gene"})
    )
    long["value"] = long["value"].astype(float)
    fl = (
        frac_df.stack().rename("size").reset_index()
    ...
    plot_kws.update(kwargs)
    base = dict(
        data=long, x="gene", y=groupby, value="value", c="value", s="size",
